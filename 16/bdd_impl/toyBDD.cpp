...
unsigned toyBDD::complement_(unsigned pRoot)
{
    ...
    unsigned lCacheIndex = hash(mComplement, pRoot, pRoot, mBinCBitNr);
    if (mBinCache[lCacheIndex].op == mComplement && mBinCache[lCacheIndex].root1 == pRoot)
        return mBinCache[lCacheIndex].result;
    
    unsigned lResult = insert(mNodes[pRoot].var,
